...
def main():
    ...
    parser.add_argument('--input_dir_images', help='input images directory')
    ...
    out_ann_file = osp.join(args.output_dir, 'annotations.csv')
    annotations = []
    for image_id, label_file in enumerate(label_files):
        ...
        img_file = osp.join(
            ...
            args.input_dir_images, label_data['imagePath'].split('/')[-1]
        )
        ...
        for i, (label, segmentation) in enumerate(segmentations.items()):
            for (xmin, ymin, xmax, ymax) in segmentation:
                annotations.append({
